...
import Banner from "@/app/(root)/App_Chunks/Components/Banner";
import data from "@/app/(root)/App_Chunks/Components/freezone";
import { useRouter } from "next/navigation";
import Link from "next/link";
import BreadCrumb from "@/app/(root)/App_Chunks/Components/BreadCrumb";
        ...
        <div className="my-10 grid grid-cols-1 lg:grid-cols-3 gap-5">
          {processData?.secondPoints.map((elem, index) => (
            <div key={index} className="w-full rounded-lg bg-[#C2DAC2] p-5">
              ...
              <div className=" mt-6">
                <p className="font-SplineSans text-xl font-semibold">
                  {elem.title}
                </p>
        ...
        <div className="mt-10 grid grid-cols-1 gap-5">
          {processData?.fourthPoints.map((item, index) => (
            ...
            >
              ...
              <p className="font-SplineSans font-medium text-xl mt-3">
                {item.title}
              </p>
              ...
              {item.isPoints &&
                (item as { points: string[] }).points?.map((point, id) => (
                  <div className="flex items-start gap-2" key={id}>
                    <div>
                      {" "}
                      <div className="size-2 mt-2 bg-lime-700 rounded-full" />
                    </div>
                    {point}
                  </div>
                ))}
            ...
            <tbody>
              {processData?.sixthTableBody.map((item, index) => (
                <tr key={index} className="font-Satoshi">
                  ...
                  {serviceName?.toLowerCase() && "doc1" in item ? (
                    <td className="border border-gray-300  ">
                      <ul>
