...
const Footer: React.FC = () => {
  const renderWidgetMenuItem = (menu: WidgetFooterMenu, index: number) => {
    return (
      <div key={index} className="text-sm">
        <h2 className="text-lg font-semibold font-Satoshi text-foreground ">
          {menu.title}
        </h2>
        <ul className="mt-5 ml-3 list-disc space-y-4">
          {menu.menus.map((item: any, index) => (
            <li key={index} className="px-1 text-foreground/90">
              <Link
                href={
                ...
                className="rounded-xl py-1 text-foreground/90 hover:bg-indigo-200 px-3 hover:text-gray-900 "
              >
                {item.label}
              </Link>
            </li>
  ...
  return (
    <>
      <div className=" relative border-t border-sky-200 bg-teal-50 overflow-hidden group py-24  lg:py-28">
